...
class LongestWordGame {
  ...
  updateStats(stats, history, lastPlay) {
    return new Promise((resolve) => {
      ...
      newStats.wins = newStats.wins.sort((a, b) => (b.wins - a.wins)); // reverse sort
  ...
  static formatStats(stats) {
    let retVal = `*${config.name} Stats:*\n` +
      `Plays: *${stats.plays}*\n` +
      `Longest Winning Word: *${stats.records.longest.word}* (<@${stats.records.longest.user}>)\n` +
      `Shortest Winning Word: *${stats.records.shortest.word}* ` +
      `(<@${stats.records.shortest.user}>)\n\n` +
      '*Top 5:*\n--------------------------------------------------\n';

    retVal += stats.wins.splice(0, 5).reduce((prev, current) => (
        `${prev}\n<@${current.user}>: ${current.wins}`
      ), '');
