...
class Bot {
  ...
  constructor(config) {
    ...
    this.games.set(mathGame.config.name, mathGame);
    ...
    if (message.text) {
      if (/^help$/i.test(message.text)) {
        ...
        if (strHelp && strHelp.length > 1) {
          const GameType = this.games.get(strHelp[1]);
          if (GameType) {
        ...
        if (strGames && strGames.length > 1) {
          ...
          if (GameType) {
            const key = GameType.config.unique.global ? gameName : `${message.channel}-${gameName}`;
            if (!this.activeGames.has(key)) {
              ...
              this.bot.reply(message,
                `Another instance of ${gameName} is already running ${location}`
              );
            }
          } else {
