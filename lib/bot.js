...
class Bot {
  ...
  constructor(config) {
    ...
    // Load Storage
    const storageDriver = storage.hasOwnProperty(config.storage.type) ?
      config.storage.type : STORAGE_DEFAULT;
      ...
      if (/^help$/i.test(message.text)) {
        ...
        if (strGames && strGames.length > 1) {
          ...
          if (GameType) {
            this.storage.getStats(GameType.config.name)
              .then((stats) => {
                this.bot.reply(message, GameType.Game.formatStats(stats, this.lookup));
              }).catch((err) => {
